...
import { EditNameModal, EditPhoneModal, EditPasswordModal } from './ProfileEditModals'
  ...
  return (
    <main id="maincontent" style={{ marginTop: '32px', marginBottom: '32px', paddingLeft: '80px', paddingRight: '80px' }}>
      <div>
        <div className="px-md md:px-xl lg:px-6xl py-0 mt-xl mb-xl">
          ...
          <div className="group/slick-carousel relative min-w-0 flex-1 max-w-full">
            ...
            <div 
              className="flex flex-row flex-nowrap overflow-x-auto no-scrollbar items-center transform-gpu scroll-smooth mb-lg max-md:mt-md py-2 gap-x-xs"
              style={{ willChange: 'scroll-position', WebkitOverflowScrolling: 'touch' }}
            >
              <Link
                href={`${basePath}/profile`}
                className={`pointer-events-auto ease-ease inline-block uppercase text-center outline-none hover:border-utility-hover disabled:text-utility-disabled focus-visible:ring-2 ring-utility-focus ring-offset-2 transition-colors duration-300 ease-ease shrink-0 py-xs md:py-sm type-utility-2 px-8 ${
                  isActive('/profile') 
                    ? 'bg-white text-black border-black border-[1px] border-solid' 
                    : 'bg-background-light text-content border-transparent hover:bg-background-dark hover:text-content-inv'
                }`}
          ...
          {(isActive('/profile') || pathname === basePath || pathname === `${basePath}/`) && (
            <div className="flex max-lg:flex-col pt-lg mt-lg border-t border-black gap-6xl">
              <div className="lg:w-6/12">
                <h1 className="type-heading-3 text-content" style={{ paddingBottom: '24px', fontSize: '1.5rem', fontWeight: 'bold' }}>PROFILE</h1>
                ...
                <div style={{ marginTop: '48px' }}>
                  ...
                  <p className="type-body-2 text-content mb-xxs">
                    {profileData?.phoneNumber 
                      ? (() => {
                          const country = COUNTRIES.find(c => c.code === profileData.phoneCountryCode)
                          const code = country?.phoneCode || ''
                          return code ? `${code} ${profileData.phoneNumber}` : profileData.phoneNumber
                        })()
                      : 'Not set'}
                  </p>
                  <button 
                    ...
                    type="button"
                  >
                    <span className="flex justify-center items-center gap-xxs preserve-line-height">{profileData?.phoneNumber ? 'Edit Phone' : 'Add Phone'}</span>
                  </button>
                </div>
                <div style={{ marginTop: '48px', paddingBottom: '32px' }}>
                  ...
                  <div className="flex items-center">
                    ...
                    <Link
                      href={`${basePath}/profile`}
                      className="pointer-events-auto transition-[color] ease-ease duration-300 focus-visible:ring-1 ring-utility-focus ring-offset-4 outline-none capitalize hover:text-utility-hover pt-xs underline type-body-3"
                    ...
                    <p className="type-body-2 text-content pt-xs ml-xxs">for exclusive perks</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <EditPhoneModal
        ...
        currentPhoneCountryCode={profileData?.phoneCountryCode}
        currentPhoneNumber={profileData?.phoneNumber}
        onSave={handleProfileUpdate}
      />
      <EditPasswordModal
